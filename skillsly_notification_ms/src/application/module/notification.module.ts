...
import {
  RabbitMQNotificationMessageClientAdapter
} from '@infrastructure/adapter/messaging/rabbit-mq/message-client/rabbitmq_notification.message_client_adapter'
...
const persistence_providers: Array<Provider> = [
  {
    ...
    useFactory: (notification, notification_resource, notification_change) =>
      new TypeOrmNotificationRepositoryAdapter(notification, notification_resource, notification_change),
    inject: [
  ...
  },
  {
    ...
    useFactory: connection => connection.getCustomRepository(TypeOrmNotificationRepository),
    inject: [Connection]
  },
  {
    provide: TypeOrmDITokens.NotificationResourceRepository,
    useFactory: connection => connection.getCustomRepository(TypeOrmNotificationResourceRepository),
    inject: [Connection]
  },
  {
    provide: TypeOrmDITokens.NotificationChangeRepository,
    useFactory: connection => connection.getCustomRepository(TypeOrmNotificationChangeRepository),
    inject: [Connection]
  }
];
...
const messaging_providers: Array<Provider> = [
  {
    ...
    useFactory: (client) => new RabbitMQNotificationMessageClientAdapter(client),
...
const use_case_providers: Array<Provider> = [
  {
    ...
    inject: [NotificationDITokens.NotificationRepository, NotificationDITokens.NotificationMessageClient]
  },
];
