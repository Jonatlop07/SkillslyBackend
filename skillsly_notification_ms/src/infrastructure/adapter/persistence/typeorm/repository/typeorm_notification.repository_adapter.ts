...
export class TypeOrmNotificationRepositoryAdapter implements NotificationRepository {
  private readonly logger: Logger = new Logger(TypeOrmNotificationRepositoryAdapter.name);
  ...
  public async create(notification_details: NotificationDetails): Promise<Array<NotificationDTO>> {
    ...
    return (
      await Promise.all(notification_details.notifier_ids.map(
        (notifier_id) =>
