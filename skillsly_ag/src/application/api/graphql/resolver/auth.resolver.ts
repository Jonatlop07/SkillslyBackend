...
export class AuthResolver {
  ...
  @Args({ name: 'credentials', type: () => AuthCredentials })
    ...
    const result: GraphQLLoggedInUser = await this.authentication_service.login(
      credentials,
    );
  ...
  @CurrentUser() graphql_user: GraphQLUserPayload,
    ...
    const { otp_auth_url } =
      await this.two_factor_auth_service.generateTwoFactorAuthSecret(
        graphql_user.id,
      );
    const response = context.res;
    response.setHeader('content-type', 'image/png');
    return await this.two_factor_auth_service.pipeQRCodeStream(
      response,
      otp_auth_url,
    );
  }
  ...
  @CurrentUser() graphql_user: GraphQLUserPayload,
    ...
    const is_valid_code =
      await this.two_factor_auth_service.isValidTwoFactorAuthCode(
        graphql_user.id,
        code,
      );
    if (!is_valid_code)
      throw new UnauthorizedException('Invalid authentication code');
    await this.two_factor_auth_service.activationOfTwoFactorAuth(
      graphql_user.id,
      true,
    );
    return null;
  ...
  @CurrentUser() graphql_user: GraphQLUserPayload,
    ...
    const is_valid_code = this.two_factor_auth_service.isValidTwoFactorAuthCode(
      graphql_user.id,
      code,
    );
    if (!is_valid_code)
      throw new UnauthorizedException('Invalid authentication code');
    const result: GraphQLTFALoggedInUser =
      await this.two_factor_auth_service.login(graphql_user, true);
