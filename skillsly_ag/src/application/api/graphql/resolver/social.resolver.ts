...
export class SocialResolver {
  ...
  @Args({ name: 'user_id', type: () => ID }) user_id: Id,
    ...
    this.logger.log(
      'Follow relationships successfully fetched from user service',
    );
  ...
  @Args({ name: 'user_id', type: () => ID }) user_id: Id,
    ...
    const { user_details: request_details } =
      await this.create_follow_user_request_service.execute({
        user_id,
        user_to_follow_id,
      });
    this.logger.log('Follow request successfully created in user service');
    this.logger.log(
      'Creating follow request notification in notification service...',
    );
    ...
    this.logger.log(
      'Create follow request notification successfully created in notification service',
    );
  ...
  @Args({ name: 'user_id', type: () => ID }) user_id: Id,
    ...
    const { user_details: request_details } =
      await this.update_follow_user_request_service.execute({
        user_id,
    ...
    if (accept) {
      this.logger.log(
        'Accepted follow request notification in notification service...',
      );
      await this.send_notification_service.execute({
        notification_details: {
      ...
      this.logger.log(
        'Accepted follow request notification successfully created in notification service',
      );
    }
    return FollowRequestDetailsMapper.toGraphQLModel(request_details);
  }
  ...
  @Args({ name: 'user_id', type: () => ID }) user_id: Id,
    ...
    const { user_details: request_details } =
      await this.delete_follow_user_request_service.execute({
        user_id,
        user_to_follow_id,
        is_follow_request,
      });
    ...
    this.logger.log(
      'Deleted follow request notification in notification service...',
    );
    await this.send_notification_service.execute({
      notification_details: {
    ...
    this.logger.log(
      'Deleted follow request notification successfully created in notification service',
    );
    return FollowRequestDetailsMapper.toGraphQLModel(request_details);
  }
}
